...
const TestimonialCard: React.FC<ITestimonialCardProps> = ({ testimonials }) => {
  return (
    <React.Fragment>
      {testimonials?.length !== 0 ? (
        <div className="mx-auto flex flex-wrap md:flex-nowrap gap-4 md:gap-7 lg:py-24 my-8">
          {testimonials?.map((testimony) => (
            <div
              key={testimony?.id}
              className="bg-[#F8F8F8] rounded-[26px] sm:w-[21.938rem] lg:w-[400px] h-[fit-content] p-6 lg:p-10"
            >
              ...
              <p className="text-lg lg:text-xl text-[#5F5E6C] mt-5">
                {testimony?.comment}
              </p>
            </div>
          ))}
