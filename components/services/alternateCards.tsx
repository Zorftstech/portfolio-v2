    ...
    return (
        <div className="mx-auto flex gap-14 max-w-7xl items-center justify-center flex-col lg:mt-28 mt-8 px-8">
            {service.content?.map((service, index) => (
                ...
                >
                    <div 
                     className={`w-[100%] lg:w-[520px] ${index % 2 === 0 ? 'lg:mr-12' : 'lg:ml-12'}`}
                    >
                        <h3 className="text-[28px] lg:text-[40px] leading-[38px] lg:leading-[80px] text-[#3B3D3F] font-bold">{service.title}</h3>
                        <p className="text-[16px] lg:text-[20px] leading-[24px] lg:leading-[30px] text-[#5F5E6C] mt-5">{service.description}</p>
